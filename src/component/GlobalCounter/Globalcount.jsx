...
export default function Globalcount() {
  ...
  const increment = () =>{
    const countRef = ref(database, "globalCount");
    set(countRef, trueSpan + 1)
  }

  const handleLike = () =>{
    if(localStorage.getItem("vote-like")) return alert("You'va already liked this post");

    const newLike = trueSpan + 1;
    const countRef = ref(database, "globalCount");
    set(countRef, newLike);
    settrueSpan(newLike)
    localStorage.setItem("voted-like", "true")
  };


  const handleDislike = () =>{
    if(localStorage.getItem("vote-dislike")) return alert("You'va already disliked this post");
